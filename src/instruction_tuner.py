...
def parse_args():
    parser = argparse.ArgumentParser(
        description="Instruction Tune an auto-regressive language model like Mistral, Llama2, Falcon, GPT-2 etc."
    )
    ...
    parser.add_argument(
        "--load_data_from_disk",
        action="store_true",
        help="Whether to load data from disk",
    )
    parser.add_argument(
        "--dataset_name_or_path",
        default="anonymous/flan2022",
        help="Dataset name(in 🤗 datasets hub) or path to a local dataset",
    )
    ...
    parser.add_argument(
        "--hf_access_token", type=str, default="", help="HuggingFace access token"
    )
    parser.add_argument(
        "--model_name_or_path",
        type=str,
        default="mistralai/Mistral-7B-v0.1",
        help="Model name or path to local checkpoint",
    )
    parser.add_argument(
        "--sliding_window",
        type=int,
        default=4096,
        help="Sliding window size in case of Mistral",
    )
    parser.add_argument(
        "--torch_dtype",
    ...
    parser.add_argument(
        ...
        action="store_true",
        help="Whether to use FlashAttention2",
    )

    parser.add_argument(
        "--load_in_8bit",
        action="store_true",
        help="Whether to enable 8-bit quantization in 8bit",
    )
    parser.add_argument(
        "--load_in_4bit",
        action="store_true",
        help="Whether to enable 4-bit quantization in 4bit",
    )
    parser.add_argument(
        "--llm_int8_threshold",
        type=float,
    ...
    parser.add_argument(
        ...
        action="store_true",
        help="Whether to enable fp32 cpu offload in LLM.int8()",
    )
    parser.add_argument(
        "--llm_int8_has_fp16_weight",
        action="store_true",
        help="Whether to run LLM.int8() with 16-bit main weights.",
    )
    parser.add_argument(
        "--bnb_4bit_compute_dtype",
        choices=["float32", "float16", "bfloat16", "auto"],
        default=None,
        help="Compute dtype for 4bit quantization",
    )
    parser.add_argument(
        "--bnb_4bit_quant_dtype",
        choices=["fp4", "nf4"],
        default="fp4",
        help="Quant dtype for 4bit quantization",
    )
    parser.add_argument(
        "--bnb_4bit_use_double_quant",
        action="store_true",
        help="Whether to use double quantization in 4bit",
    )

    # Parameters related to preprocessing
    parser.add_argument(
        "--max_seq_length", type=int, default=512, help="Input Sequence Length"
    )
    parser.add_argument(
        "--overwrite_cache",
        action="store_true",
    ...
    parser.add_argument(
        "--learning_rate", default=2.5e-5, type=float, help="Learning rate"
    )
    parser.add_argument(
        "--per_device_train_batch_size",
        type=int,
        default=4,
        help="The per-device train batch size",
    )
    parser.add_argument(
        "--per_device_eval_batch_size",
        type=int,
        default=4,
        help="The per-device eval batch size",
    )
    parser.add_argument(
    ...
    parser.add_argument(
        "--peft_lora_dropout", type=float, default=0.05, help="LoRA dropout"
    )
    parser.add_argument(
        "--peft_target_modules",
        default="q_proj,k_proj,v_proj,o_proj,gate_proj,up_proj,down_proj,lm_head",
        help="LoRA target modules - comma separated with no spaces",
    )
    ...
    parser.add_argument(
        "--adamw_fused", action="store_true", help="Whether to set fused=True in AdamW"
    )
    parser.add_argument(
        "--lr_scheduler_type",
        type=SchedulerType,
    ...
    parser.add_argument(
        "--merge_weights",
        action="store_true",
        help="Whether to merge weights at the end of training",
    )
    parser.add_argument(
        "--push_to_hub",
        action="store_true",
    ...
    parser.add_argument(
        "--private_repo",
        action="store_true",
        help="Whether the created repo is private or not",
    )
...
def main():
    ...
    if accelerator.is_local_main_process:
        if args.push_to_hub:
            ...
            repo_id = create_repo(
                repo_name, exist_ok=True, token=args.hub_token, private=is_private
            ).repo_id
            # Clone repo locally
        ...
        with open(args.dataset_name_or_path, 'r') as f:
            ...
            raw_dataset = DatasetDict({
                "train" : Dataset.from_list(raw_data["train"]),
                 "validation" : Dataset.from_list(raw_data["validation"]) if len(raw_data["validation"]) > 0 else Dataset.from_list(raw_data["train"]),
    ...
    def preprocess_function(examples):
        ...
        prompts_responses=[p+" "+r for p, r in zip(examples["inputs"], examples["targets"])]
        prompts_responses_tokenized=tokenizer(prompts_responses, truncation=True, max_length=max_seq_length)
        prompts_tokenized=tokenizer(examples["inputs"], truncation=True, max_length=max_seq_length)
